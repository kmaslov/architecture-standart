...
1. Kafka это новая технология в IT-ландшафте, разработчики могут не иметь опыта работы с ней.
1. В процессе получения ставок будут прямые запросы из сервиса интернет-банка, который доступен публично, в сервис АБС, который расположен во внутреннем контуре и не должен быть доступен извне. Это риск в контексте безопасности, следует продумать меры по снижению этого риска с командой ИБ (например, ограничение доступа по IP, api key, лимиты на количество запросов).
1. Сервис обработки депозитов пока будет ходить в основную базу АБС. С ростом нагрузки эту связь стоит развязать.
